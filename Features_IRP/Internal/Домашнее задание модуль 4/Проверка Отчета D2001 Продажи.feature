...
* Проверка не попадания в отчет документа помеченного на удаление
	...
	И я выполняю код встроенного языка на сервере
		|'Документы.SalesReturn.НайтиПоНомеру(3).ПолучитьОбъект().УстановитьПометкуУдаления(Истина);'|
	И В командном интерфейсе я выбираю "Продажи" "Возвраты товаров от покупателей"
	И я нажимаю на кнопку с именем 'FormRefresh'
	Когда В панели открытых я выбираю "D2001 Продажи (Отчет с регистратором)"
	Тогда открылось окно "D2001 Продажи (Отчет с регистратором)"
	И я нажимаю на кнопку с именем 'FormGenerate'
	Дано Табличный документ "Result" равен макету "ЭталонОтчетаПродажиСОтборомПоВозвратам" по шаблону
